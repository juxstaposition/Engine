...
VideoLoader::VideoLoader()
{
    ...
    {
        FloodFill(_frame, _fieldArea, 25, 70);
    }
...
void VideoLoader::FloodFill(cv::Mat& img, cv::Mat& result, int thLow, int thHigh)
{
    ...
    cv::Mat resizedMask, imgResult, resizedMaskDilate;
    ...
    //    {
    //        if ((i < del || i > maxX - del) && (j < del || j > maxY - del) )
    //        {
    //            uchar& val = mask.at<uchar>(i,j, CV_8UC1);
    //            val = 0;
    ...
    cv::resize(mask, resizedMask, cv::Size(img.cols, img.rows));
    cv::dilate(resizedMask, resizedMaskDilate, _kernelElement);

    //cv::bitwise_and(img, img, imgResult, resizedMaskDilate);

    result = resizedMaskDilate.clone();
